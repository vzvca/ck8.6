    ...
    char **argv;		/* Argument strings. */
{
    ...
    Tcl_SetObjResult( interp, Tcl_NewStringObj(scrollPtr->winPtr->pathName,-1));
    return TCL_OK;
    ...
    char **argv;			/* Argument strings. */
{
	...
	Tcl_AppendResult(interp, first, " ", last, (char *) NULL);
        ...
        switch (thing) {
	case TOP_ARROW:     Tcl_SetObjResult( interp, Tcl_NewStringObj( "arrow1", -1));;      break;
	case TOP_GAP:       Tcl_SetObjResult( interp, Tcl_NewStringObj( "trough1", -1));     break;
	case SLIDER:        Tcl_SetObjResult( interp, Tcl_NewStringObj( "slider", -1));      break;
	case BOTTOM_GAP:    Tcl_SetObjResult( interp, Tcl_NewStringObj( "trough2", -1));     break;
	case BOTTOM_ARROW:  Tcl_SetObjResult( interp, Tcl_NewStringObj( "arrow2", -1));      break;
        }
