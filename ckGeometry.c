...
void
Ck_RecomputeLayout(mainWinPtr)
    CkWindow *mainWinPtr;		/* Pointer to main window of interpreter */
{
  CkWindow *winPtr;

  if ( (mainWinPtr->geomMgrPtr != NULL) && (mainWinPtr->geomMgrPtr->requestProc != NULL) ) {
	(*mainWinPtr->geomMgrPtr->requestProc)(mainWinPtr->geomData, mainWinPtr);
  }
  else {
    for( winPtr = mainWinPtr->childList; winPtr != NULL; winPtr = winPtr->nextPtr ) {
      if ( (winPtr->geomMgrPtr != NULL) && (winPtr->geomMgrPtr->requestProc != NULL) ) {
	(*winPtr->geomMgrPtr->requestProc)(winPtr->geomData, winPtr);
      }
    }
  }
