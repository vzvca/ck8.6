...
int
Ck_OptionCmdObj(clientData, interp, objc, objv)
     ClientData clientData;     /* Main window associated with
                                 * interpreter. */
     Tcl_Interp *interp;                /* Current interpreter. */
     ...
     Tcl_Obj* CONST objv[];     /* Tcl_Obj* array of arguments. */
{
  ...
  if (Tcl_GetIndexFromObj( interp, objv[1], commands, "command", TCL_EXACT, &index) != TCL_OK) {
    return TCL_ERROR;
  }
  ...
  case CMD_GET:
	...
	Tcl_WrongNumArgs(interp, 2, objv, "window name class");
      ...
      winPtr2 = Ck_NameToWindow(interp, Tcl_GetString(objv[2]), winPtr);
      if (winPtr2 == NULL) {
        return TCL_ERROR;
      }
      value = Ck_GetOption(winPtr2, Tcl_GetString(objv[3]), Tcl_GetString(objv[4]));
      if (value != NULL) {
        Tcl_SetObjResult( interp, Tcl_NewStringObj(value,-1));
      }
      return TCL_OK;
  ...
  case CMD_READFILE:
    {
      ...
      if ((objc != 3) && (objc != 4)) {
	Tcl_WrongNumArgs(interp, 1, objv, "fileName ?priority?");
        return TCL_ERROR;
      }
      if (objc == 4) {
